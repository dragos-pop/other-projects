...
class CreateDataset:
    ...
    def add_numerical_dataset(self, file, timestamp_col, value_cols, aggregation='avg', prefix=''):
        ...
        for i in range(0, len(self.data_table.index)):
            ...
            for col in value_cols:
                # Take the aggregated value
                if len(relevant_rows) > 0:
                    if aggregation == 'avg':
                        self.data_table.loc[self.data_table.index[i], str(prefix)+str(col)] = np.average(relevant_rows[col])
                    elif aggregation == 'max':
                        self.data_table.loc[self.data_table.index[i], str(prefix)+str(col)] = relevant_rows[col].max()
                    elif aggregation == 'min':
                        self.data_table.loc[self.data_table.index[i], str(prefix)+str(col)] = relevant_rows[col].min()
                    elif aggregation == 'std':
                        self.data_table.loc[self.data_table.index[i], str(prefix)+str(col)] = np.std(relevant_rows[col])
                    else:
...
if __name__ == "__main__":
    ...
    aggregation_methods = ['avg', 'std', 'min', 'max']
    dataset = CreateDataset(Path('.'), granularity = miliseconds_per_instance)

    for aggregation_method in aggregation_methods:
        dataset.add_numerical_dataset('data/Accelerometer.csv', 'Time (s)', ['X (m/s^2)','Y (m/s^2)','Z (m/s^2)'], aggregation_method, f'acc_{aggregation_method}_')
        dataset.add_numerical_dataset('data/Barometer.csv', 'Time (s)', ['X (hPa)'], aggregation_method, f'bar_{aggregation_method}_')
        dataset.add_numerical_dataset('data/Gyroscope.csv', 'Time (s)', ['X (rad/s)', 'Y (rad/s)', 'Z (rad/s)'], aggregation_method, f'gyr_{aggregation_method}_')
        dataset.add_numerical_dataset('data/Linear Accelerometer.csv', 'Time (s)', ['X (m/s^2)','Y (m/s^2)','Z (m/s^2)'], aggregation_method, f'lin_acc_{aggregation_method}_')
        dataset.add_numerical_dataset('data/Location.csv', 'Time (s)', ["Latitude (°)","Longitude (°)","Height (m)","Velocity (m/s)","Direction (°)","Horizontal Accuracy (m)","Vertical Accuracy (°)"], aggregation_method, f'loc_{aggregation_method}_')
        dataset.add_numerical_dataset('data/Magnetometer.csv', 'Time (s)', ['X (µT)', 'Y (µT)', 'Z (µT)'], aggregation_method, f'mag_{aggregation_method}_')
        dataset.add_numerical_dataset('data/Proximity.csv', 'Time (s)', ['Distance (cm)'], aggregation_method, f'prox_{aggregation_method}_')

    dataset.data_table.index = dataset.data_table.index + pd.Timedelta(weeks = 2788, days = 1, hours=9, minutes=6, seconds=10)
    dataset.data_table.to_csv(f'data/data_agg.csv')
